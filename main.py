...
from utilities import provjera_korisnickog_unosa, provjera_unosa_artikla, provjera_unosa_prodaje
...
# Frame korisnik
        ...
        self.frame_label_korisnik.setGeometry(QtCore.QRect(0, 0, 61, 30))
        ...
        self.frame_prodaja.setGeometry(QtCore.QRect(245, 300, 510, 195))
...
# Odabir i unos korisnika
    ...
    def unos_korisnika(self):
        if self.tip_korisnika.currentText() == TipKorisnika.PRIVATNI.value:
            error_k = provjera_korisnickog_unosa(self.text_telefon.text(), self.text_email.text(), self.text_ime.text(),
                                               self.text_prezime.text())

        elif self.tip_korisnika.currentText() == TipKorisnika.POSLOVNI.value:
            error_k = provjera_korisnickog_unosa(self.text_telefon.text(), self.text_email.text(), self.text_naziv.text(),
                                               self.text_web.text())
